...
export const getArticleRecommends = recommendAdapter.getSelectors<StateSchema> (
    (state) => state.articleDetailsPage?.recommends || recommendAdapter.getInitialState()
)

const articleDetailsRecommendsSlice = createSlice({
    name: 'articleDetailsRecommendsSlice',
    initialState: recommendAdapter.getInitialState<ArticleDetailsRecommendsSchema>({
        isLoading: false,
        error: undefined,
        ids: [],
        entities: {},
